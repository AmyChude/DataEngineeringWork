...
from connectors import get_alpaca_api_data, get_database_engine, write_to_database
...
def extract(api_key_id: str, api_secret_key: str, stock_ticker: str, start_date: str, end_date: str):
    ...
    for dates in _generate_datetime_ranges(start_date=start_date, end_date=end_date):
        ...
        data = get_alpaca_api_data(stock_ticker=stock_ticker, start_time=start_time, end_time=end_time, api_key_id=api_key_id, api_secret_key=api_secret_key)
...
def load(df_exchange: pd.DataFrame, db_user: str, db_password: str, db_server_name: str, db_database_name: str):
    engine = get_database_engine(db_user=db_user, db_password=db_password, db_server_name=db_server_name, db_database_name=db_database_name)
    write_to_database(engine=engine, df_exchange=df_exchange)    
