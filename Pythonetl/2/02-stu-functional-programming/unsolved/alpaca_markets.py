...
def extract_data()-> list[dict]:
   # df_exchange_codes = pd.read_csv("csv_path")
...
def extract_exchange_codes(csv_path: str) -> pd.DataFrame: 
    exchange = pd.read_csv(csv_path)
    return exchange

def transform_data(trades: list[dict], exchange: pd.DataFrame) -> pd.DataFrame:
    # read json data to a dataframe 
    df_quotes = pd.json_normalize(data=trades, meta=["symbol"])
    ...
    df_exchange = pd.merge(left=df_quotes_selected, right=exchange, left_on="exchange", right_on="exchange_code").drop(columns=["exchange_code", "exchange"]).rename(columns={"exchange_name": "exchange"})
    return df_exchange


def load_data(df_exchange: pd.DataFrame):
    ...
    return engine.execute(upsert_statement).all()

if __name__ == "__main__": 
    trades_data = extract_data()
    exchange_data = extract_exchange_codes("data/exchange_codes.csv")
    df = transform_data(trades=trades_data, exchange=exchange_data)
    load_data(df)
