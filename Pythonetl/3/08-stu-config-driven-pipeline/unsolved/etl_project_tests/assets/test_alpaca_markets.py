...
def test_extract_alpaca_markets(setup_extract):
    ...
    df = extract_alpaca_markets(alpaca_markets_client, "TSLA", "2023-02-06", "2023-02-8")
    assert len(df) == 2000
...
def setup_exchange_codes_df():
    return extract_exchange_codes("etl_project_tests/data/alpaca_markets/exchange_codes.csv")

@pytest.fixture
def setup_transformed_alpaca_df():
    return pd.DataFrame([
        {"id": 1, "timestamp": 1, "price": 10, "size": 100, "exchange": "NYSE American (AMEX)"},
        {"id": 2, "timestamp": 2, "price": 20, "size": 200, "exchange": "NASDAQ OMX BX"},
        {"id": 3, "timestamp": 3, "price": 30, "size": 300, "exchange": "National Stock Exchange"}
    ])

def test_transform(setup_input_alpaca_df, setup_exchange_codes_df, setup_transformed_alpaca_df):
