...
class AlpacaMarketsApiClient:
    ...
    def get_trades(self, stock_ticker: str, start_time: str, end_time: str) -> list[dict]:
        """
        ...
        if response.status_code == 200 and response.json().get("trades") is not None: 
            ...
            raise Exception(f"Failed to extract data from Alpaca API. Status Code: {response.status_code}. Response: {response.text}")
